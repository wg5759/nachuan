...
- SDK 与生态 bridge：PK-008 的 `nachuan_sdk` 只构建现有 verifier 接受的签名三文件 `transform.json` bundle，不写签名私钥。DeepSeek Harness 与 OpenClaw 只按精确官方 commit 解析有界配置/manifest/Skill 形态，原文敏感配置和 Skill 正文不进入宿主投影；浮动 ref、YAML alias/custom tag、重复 JSON 字段、路径逃逸、未知执行字段和过大输入均拒绝。Bridge runtime 必须借用已验隔离代理，worker 只能返回原组件 ID 的闭集投影；语义越界会触发精确身份 quarantine。默认未挂 proxy/worker，健康页固定 `marketplace_enabled=false/production_ready=false`。这不等于 OpenClaw/Cordis 原生代码、MCP、Hook、provider/channel/tool/UI 已获准，也不等于公共市场、自动下载/信任/升级或 Linux/macOS 隔离已完成。
- 本地 Web 持久登录：runtime/approval Key 继续只在当前用户 DPAPI 文档和 Engine 内存中存在，不再打印到隐藏终端。CLI 绑定专属回环 `127.77.77.77`，生成120秒单次 bootstrap capability，只放在浏览器 fragment；页面第一时间用 `history.replaceState` 清除 fragment，并经 exact Host/port、`X-Nachuan-Web-Session: 1`、`Sec-Fetch-Site: same-origin`、POST Origin 和闭集 JSON 换取 host-only、HttpOnly、SameSite=Strict Cookie。Cookie 180天且每次成功状态探测续期；Cookie 鉴权只在同一专属回环 host 和自定义同源头同时成立时接受，`127.0.0.1` 或冲突 Bearer/审批头拒绝。手工 Key 只作高级回退，验证两把 Key 后可升级 Cookie。该边界避免 localStorage 明文和每标签页重复输入；同一 Windows SID 的恶意进程仍不在本地 Web Cookie 的隔离保证内，不能据此宣称抵御本机 owner 级恶意软件。
